...
export async function setLaboStatut(client: Client, guildId: string, laboKey: string, available: boolean, tempsRestantMinutes = 0): Promise<void> {
  ...
  try {
    ...
    // Un temps restant de 0 (labos sans délai de production, ex. Salvia,
    // Branche De Cannabis) équivaut à rester disponible — jamais de passage
    // au rouge sans timer pour en sortir (sinon le salon resterait bloqué).
